...
Test Damsel
```{r}
makeDGE2(damsel_counts_a, min.samples = 2)$design
testDmRegions(makeDGE2(damsel_counts_a, min.samples = 2), gatc_regions) %>% 
  group_by(meth_status) %>% summarise(n=n())
```
test again but without incorporating replicates into the design matrix
```{r}
...
testDmRegions(makeDGE2(damsel_counts_a, min.samples = 2), gatc_regions) %>% 
  group_by(meth_status) %>% summarise(n=n())
```
Test again but without incorporating replicates into design matrix
```{r}
makeDGE2(damsel_counts_a, min.samples = 2, include_replicate=FALSE)$design
testDmRegions(makeDGE2(damsel_counts_a, min.samples = 2, include_replicate=FALSE), gatc_regions) %>% 
  group_by(meth_status) %>% summarise(n=n())
